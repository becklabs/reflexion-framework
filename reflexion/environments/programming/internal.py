...
class InternalTestingEnv(TestingEnv):
    def __init__(
        ...
        self.test_generation_function: LanguageFunction = (
            LanguageFunction.from_yaml(
                os.path.join(
                    PROMPTS_DIR, "programming", "v1", "testing", language, "assumptions.yaml"
                ),
                llm=llm
            )
        )
    ...
    def _generate_tests(self):
        function_response = self.test_generation_function(docstring = self.docstring, function_signature = self.function_signature)
